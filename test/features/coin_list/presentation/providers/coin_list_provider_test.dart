...
import 'package:coin_tracker_app/src/features/coin_list/data/models/time_period_model.dart';
...
void main() {
  ...
  late final MockGetListOfTimePeriods getListOfTimePeriods;
  ...
  setUpAll(() {
    ...
    getListOfTimePeriods = MockGetListOfTimePeriods();
    provider = CoinListProvider(
        ...
        getListOfTimePeriods: getListOfTimePeriods);
  });
  ...
  group('provider usecases', () {
    ...
    test('provider should get a list of exchange data from asset', () async {
      ...
      expect(provider.historicExchangeRate.length, 2);
      expect(verifyEqualLists, true);
    });

    test('provider should get a list of TimePeriods from asset', () async {
      when(getListOfTimePeriods.call(NoParams())).thenAnswer((_) async {
        final Map<String, dynamic> jsonMap = json.decode(fixture('time_period.json'));
        return Right(<TimePeriod>[
          TimePeriodModel.fromJson(jsonMap),
          TimePeriodModel.fromJson(jsonMap),
        ]);
      });

      await provider.loadTimePeriodList();

      final Map<String, dynamic> jsonMap = json.decode(fixture('time_period.json'));
      bool verifyEqualLists = listEquals(provider.timePeriods, [
        TimePeriodModel.fromJson(jsonMap),
        TimePeriodModel.fromJson(jsonMap),
      ]);
      expect(verifyEqualLists, true);
